...
export default function HtmlEditor() {
  ...
  const handleUpload = async (file: File) => {
    ...
    const blobUrl = URL.createObjectURL(new Blob([processed.htmlForPreview], { type: "text/html" }));
  ...
  const handleImageChange = async (file: File) => {
    ...
    if (idx !== -1) {
      ...
      setAttributes([...attributes, { name: "src", value: zipFilePath, blobUrl }]);
    }
  };
  ...
  return (
    <Flex p={6} gap={6} h="100vh" bg={appBg} fontFamily="Inter, sans-serif" ref={containerRef}>
      <PreviewPane onUpload={handleUpload} iframeUrl={iframeUrl} iframeRef={iframeRef} />
      {/* split handle */}
      <Box
        role="separator"
